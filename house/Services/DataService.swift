...
extension DataService {
    ...
    /// Returns the amount of chores for a given house
    public func getAmountOfChores(houseId: String, completion: @escaping (_ amount: Int) -> ()) {
        self.REF_CHORES.child(houseId).observe(.value, with: { (snapshot) in
            if let snapshot = snapshot.children.allObjects as? [DataSnapshot] {
            var amount = 0
            for _ in snapshot { amount += 1 }
...
extension DataService {
    ...
    /// Returns the amount of shopping items for a given house
    public func getAmountOfShoppingItems(houseId: String, completion: @escaping (_ amount: Int) -> ()) {
        self.REF_SHOPPING.child(houseId).observe(.value, with: { (snapshot) in
            guard let snapshot = snapshot.children.allObjects as? [DataSnapshot] else { return }
...
extension DataService {
    ...
    /// Returns the amount of outstanding debts that a given user has within a given house
    public func getAmountOfOutstandingDebts(userId: String, houseId: String, completion: @escaping (_ amount: Int) -> ()) {
        self.REF_DEBTS.child(houseId).observe(.value, with: { (snapshot) in
            guard let snapshot = snapshot.children.allObjects as? [DataSnapshot] else { return }
